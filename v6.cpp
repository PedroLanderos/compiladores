...
struct Node 
{
    string familia;
    string body;
    unordered_map<string, int> variables; // Incluye variables locales y globales
...
void CheckVariableDeclaration(unordered_map<string, int>& variables, string var)
{
    if (variables.find(var) != variables.end())
    {
        cout << "Variable mal declarada o repetida: " << var << endl;
        exit(1);
    }
}

void CheckVariableUsage(unordered_map<string, int>& variables, string var)
{
    if (variables.find(var) == variables.end())
    {
        ...
        if (regex_search(code, match, regex(families["V"])))
        {
            if (match.position() == 0)  
            {
                ...
                for (size_t i = 0; i < tokens.size(); ++i) 
                {
                    ...
                    {
                        string varName = tokens[i].first;
                        CheckVariableDeclaration(root.variables, varName);
                ...
                if (startParams != string::npos && endParams != string::npos && startParams < endParams)
                {
                    ...
                    for (size_t i = 0; i < paramParsedTokens.size(); ++i)
                    {
                        if (paramParsedTokens[i].second == "D" && i + 1 < paramParsedTokens.size() && paramParsedTokens[i + 1].second == "N")
                        {
                            ...
                            CheckVariableDeclaration(functionNode->variables, varName);
    ...
    while(!code.empty())
    {
                ...
                if (startParams != string::npos && endParams != string::npos && startParams < endParams)
                {
                    ...
                    for (const auto &paramToken : paramParsedTokens)
                    {
                        if (paramToken.second == "N") 
                            CheckVariableUsage(nodo.variables, paramToken.first);
                    }
                }
        ...
        if (!matched && regex_search(code, match, regex(families["V"])))
        {
            if (match.position() == 0)
            {
                ...
                for (size_t i = 0; i < tokens.size(); ++i) 
                {
                    ...
                    {
                        string varName = tokens[i].first;
                        if (nodo.variables.find(varName) != nodo.variables.end() && nodo.familia != "root")
                        {
                            cout << "Variable mal declarada o repetida: " << varName << endl;
                            exit(1);
                        }
                        nodo.variables[varName]++;
                    }
                }
                code = match.suffix().str();
                matched = true;
        ...
        if (!matched && regex_search(code, match, regex(families["VA"])))
        {
            if (match.position() == 0)
            {
                ...
                {
                    if (par.second == "N") 
                        CheckVariableUsage(nodo.variables, par.first);
                }
                code = match.suffix().str();
                matched = true;
                continue;
